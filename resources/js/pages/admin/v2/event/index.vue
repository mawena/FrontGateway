...
const apiDelete = async id => {
	...
	if (response.status == 200) {
		...
		snackbarMessage.value = "Evenement Supprimé"
	} else {
	...
	<div>
		...
		<VCard title="Filtres" class="mb-6">
			<VCardText>
				<VRow>
					<VCol cols="12" sm="12">
						<AppSelect v-model="validationFilter" placeholder="Validation" item-title="name" item-value="key"
							:items="[{'key': 'waiting', 'name': 'En attente'},{'key': 'validated', 'name': 'Validés'},{'key': 'rejected', 'name': 'Rejetés'}]" clearable clear-icon="tabler-x" />
					</VCol>
				</VRow>
