	...
	refForm.value?.validate().then(async ({ valid }) => {
		if (valid) {
			...
			if (res.status == 201) {
				...
				if (res.errors.email) {
					itemError.value["email"] = res.errors.email
					res.errors.email = null
				}
				if (res.errors.password) {
					itemError.value["password"] = res.errors.password
					res.errors.password = null
				}
				if (res.errors.activated) {
					itemError.value["activated"] = res.errors.activated
	...
	<div>
		<div class="d-flex flex-wrap justify-start justify-sm-space-between gap-y-4 gap-x-6 mb-6">
			<div class="d-flex flex-column justify-center">
				...
				<span>Informations sur l'utiliateur</span>
			</div>
		</div>
					...
					<VCard class="mb-6" title="Informations de l'utilisateur">
						<VCardText>
							<VRow>
								...
								<VCol cols="12" md="12" lg="6">
									<VTextField v-model="itemData.password" label="Mot de passe"
