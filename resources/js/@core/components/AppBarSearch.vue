	...
	<VDialog max-width="600" :model-value="props.isDialogVisible" :height="$vuetify.display.smAndUp ? '550' : '100%'"
		...
		<VCard height="100%" width="100%" class="position-relative">
			<VCardText class="pt-2">
				<!-- 👉 Search Input -->
				<VTextField ref="refSearchInput" v-model="searchQueryLocal" autofocus density="comfortable"
					variant="plain" @keyup.esc="clearSearchAndCloseDialog" @keydown="getFocusOnSearchList"
			...
			<PerfectScrollbar :options="{ wheelPropagation: false, suppressScrollX: true }" class="h-100">
				<!-- 👉 Search List -->
				<VList v-show="searchQueryLocal.length && !!props.searchResults.length" ref="refSearchList"
